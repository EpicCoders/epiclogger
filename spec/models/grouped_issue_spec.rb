...
describe GroupedIssue do
  ...
  context '#aggregations' do
    ...
    it 'returns different browser occurences' do
      ...
      aggregations = grouped_issue.aggregations("browser")
      expect(aggregations[0]["browser"]).to eq('Chrome')
      expect(aggregations[1]["browser"]).to eq('Firefox')
    end
    ...
    it 'returns different messages' do
      ...
      aggregations = grouped_issue.aggregations("message")
      expect(aggregations[0]["message"]).to eq('ZeroDivisionError: divided by 0')
      expect(aggregations[1]["message"]).to eq('mesaju vietii')
    end
    ...
    it 'returns different subscribers' do
      ...
      aggregations = grouped_issue.aggregations("subscriber")
      expect(aggregations[0]["subscriber"]["name"]).to eq(subscriber.name)
      expect(aggregations[1]["subscriber"]["name"]).to eq(subscriber2.name)
    end
  end
end
