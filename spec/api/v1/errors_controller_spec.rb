...
describe Api::V1::ErrorsController, :type => :controller do
  ...
  describe 'GET #index' do
    it 'should get current_site errors' do
      get :index, { id: issue_error.id, error: {status: issue_error.status }, format: :json }
      expect(assigns(:error)).to eq(issue_error.status)
    end

    it 'should render json' do
      get :index, { id: issue_error.id, error: { status: issue_error.status, web: issue_error.website }, format: :json }
  ...
  describe 'PUT #update' do
    ...
    it 'should not allow update of other parameters other than status' do
      ...
      expect(:update).not_to change(error.status).from(resolved)
